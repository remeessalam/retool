import Logo from "./assets/images/Logo/Logo.png";
import phonenumbersvg from "./assets/images/svg/phonenumber.svg";
import homepagebanner from "./assets/images/homepagebanner.jpeg";
import homepageaboutus from "./assets/images/homepageaboutus.jpg";
import homepageoverimageone from "./assets/images/homepageoverimageone.png";
import homepageoverimagetwo from "./assets/images/homepageoverimagetwo.jpeg";
import homepagebackgroundvideo from "./assets/video/homepagebannervideo.mp4";
import verticaldivider from "./assets/images/verticaldivider.png";
import meetcompanyimageone from "./assets/images/meetcompanyimageone.png";
import meetcompanyimagetwo from "./assets/images/meetcompanyimagetwo.png";
import meetcompanyimagethree from "./assets/images/meetcompanyimagethree.png";
import facebookIcon from "./assets/images/facebook.png";
import twitterIcon from "./assets/images/twitter.png";
import behanceIcon from "./assets/images/behance.png";
import youtubeIcon from "./assets/images/youtube.png";
import linkedinIcon from "./assets/images/linkedin.png";
import aiimage from "./assets/images/services/aiimage.png";
import mobiledevelopmentimage from "./assets/images/services/mobiledevelopmentimage.png";
import webdevelopmentimage from "./assets/images/services/webdevelopmentimage.png";
import uiuximage from "./assets/images/services/uiuximage.png";
import customsoftwareimage from "./assets/images/services/customsoftwareimage.png";
import dataanalyticsimage from "./assets/images/services/dataanalyticsimage.png";
import iotimage from "./assets/images/services/iotimage.png";
import roboticautomationimage from "./assets/images/services/roboticautomationimage.png";
import gamedevelopmentimage from "./assets/images/services/gamedevelopmentimage.png";
import blockchainimage from "./assets/images/services/blockchainimage.png";
import saiimage from "./assets/images/serviceimage/aiimage.png";
import smobiledevelopmentimage from "./assets/images/serviceimage/mobiledevelopmentimage.png";
import swebdevelopmentimage from "./assets/images/serviceimage/webdevelopmentimage.png";
import suiuximage from "./assets/images/serviceimage/uiuximage.png";
import scustomsoftwareimage from "./assets/images/serviceimage/customsoftwareimage.png";
import sdataanalyticsimage from "./assets/images/serviceimage/dataanalyticsimage.png";
import siotimage from "./assets/images/serviceimage/iotimage.png";
import sroboticautomationimage from "./assets/images/serviceimage/roboticautomationimage.png";
import sgamedevelopmentimage from "./assets/images/serviceimage/gamedevelopmentimage.png";
import sblockchainimage from "./assets/images/serviceimage/blockchainimage.png";
import aboutpageimageone from "./assets/images/aboutpageimageone.jpeg";
import companyachievementsimage from "./assets/images/companyachievementsimage.jpg";
import aboutpageimagethree from "./assets/images/aboutpageimagethree.jpeg";
import aboutpageimagefour from "./assets/images/aboutpageimagefour.jpeg";
import aboutpageimagetwo from "./assets/images/aboutpageimagetwo.png";
import fullstart from "./assets/images/fullstar.png";
import testimonialsuserimage from "./assets/images/testimonialsuserimage.png";
import contactuspagebanner from "./assets/images/contactuspagebanner.jpg";
import landingpagebannerapp from "./assets/images/landingpagebannerapp.jpg";
import landingpagebannerweb from "./assets/images/landingpagebannerweb.png";
import whychooseusimg from "./assets/images/whychooseus.png";
import ecommerce from "./assets/images/landingpage/ecommerce.png";
import socialmedia from "./assets/images/landingpage/socialmedia.png";
import landingpage from "./assets/images/landingpage/landingpage.png";
import custom from "./assets/images/landingpage/custom.png";
import iosapp from "./assets/images/landingpage/iosapp.png";
import android from "./assets/images/landingpage/android.png";
import flutter from "./assets/images/landingpage/flutter.png";
import hybrid from "./assets/images/landingpage/hybrid.png";
import mechanical from "./assets/images/landingpage/mechanical.png";
import expert from "./assets/images/landingpage/expert.png";
import target from "./assets/images/landingpage/target.png";
import cogwheel from "./assets/images/landingpage/cogwheel.png";
import webLandingAbout from "./assets/images/webLandingAbout.png";
import appLandingAbout from "./assets/images/appLandingAbout.png";
import whychooseusbackground from "./assets/images/whychooseusbackground.jpg";
import testimonialbackground from "./assets/images/testimonialbackground.jpg";
import Finetunedsolutionimg from "./assets/images/Finetunedsolutionimg.jpg";
import Innovationfocused from "./assets/images/Innovationfocused.jpg";
import Scalableframeworks from "./assets/images/Scalableframeworks.jpg";
import aboutusbanner from "./assets/images/aboutusbanner.jpg";
import servicepagebanner from "./assets/images/servicepagebanner.jpg";
import aiImage from "./assets/images/aiimageforcontact.png";
import landingaboutpc from "./assets/images/landingaboutpc.png";
import faqbg from "./assets/images/faqbg.png";
// app porffolio images
import cryptopadie from "./assets/images/portfolio/app/cryptopadie.png";
import doctorplus from "./assets/images/portfolio/app/doctorplus.png";
import dubaitravelguide from "./assets/images/portfolio/app/dubai travel guide.webp";
import elorahairpalour from "./assets/images/portfolio/app/elora hair palour.webp";
import evansfrancis from "./assets/images/portfolio/app/evans francis.webp";
import houseofdeliverance from "./assets/images/portfolio/app/house of deliverance.png";
import mybitsshop from "./assets/images/portfolio/app/mybitsshop.webp";
import namaz from "./assets/images/portfolio/app/namaz.webp";
import potea from "./assets/images/portfolio/app/potea.webp";
import rentop from "./assets/images/portfolio/app/rentop.png";
// web portfolio images
import ghomes from "./assets/images/portfolio/web/5ghomes.webp";
import Autopilot from "./assets/images/portfolio/web/Autopilot.png";
import bayut from "./assets/images/portfolio/web/bayut.webp";
import coldcreekcap from "./assets/images/portfolio/web/cold creekcap.webp";
import collegenutritionnist from "./assets/images/portfolio/web/collegenutritionnist.png";
import leadership from "./assets/images/portfolio/web/leadership.net.png";
import menissacaterings from "./assets/images/portfolio/web/menissa caterings.webp";
import thinkreality from "./assets/images/portfolio/web/think reality.webp";

export const images = {
  Logo,
  phonenumbersvg,
  homepagebanner,
  homepageaboutus,
  homepageoverimageone,
  homepageoverimagetwo,
  verticaldivider,
  aboutpageimageone,
  companyachievementsimage,
  aboutpageimagethree,
  aboutpageimagefour,
  aboutpageimagetwo,
  contactuspagebanner,
  whychooseusbackground,
  testimonialbackground,
  Finetunedsolutionimg,
  Innovationfocused,
  Scalableframeworks,
  aboutusbanner,
  servicepagebanner,
  aiImage,
  faqbg,
  swebdevelopmentimage,
  smobiledevelopmentimage,
  suiuximage,
};

export const landingpageimages = {
  landingpagebannerapp,
  landingpagebannerweb,
  whychooseusimg,
  mechanical,
  expert,
  target,
  cogwheel,
  webLandingAbout,
  appLandingAbout,
  landingaboutpc,
};
export const video = { homepagebackgroundvideo };

export const socialmediaIcons = [
  facebookIcon,
  twitterIcon,
  behanceIcon,
  youtubeIcon,
  linkedinIcon,
];

export const companyDetails = {
  address:
    "101, Oxford Towers, 139, HAL Old Airport Rd, Kodihalli, Bengaluru, Karnataka 560008",
  phone: "+91-9945973151",
  whatsapp: "9945973151",
  link: "🌐 Website: RETOOL.com",
  email: "example@abc.com",
};

export const ourServices = [
  {
    id: 1,
    img: aiimage,
    image: saiimage,
    title: "Artificial Intelligence (AI)",
    description:
      "Artificial Intelligence is more than a buzzword—it’s a game changer for businesses looking to stay ahead. descriptionAt [Your Company Name], we help you unlock the potential of AI by integrating intelligent systems that automate tasks, analyze complex data, and make smarter decisions. Whether it's chatbots, predictive analytics, or advanced machine learning models, we offer AI solutions tailored to your business challenges. By implementing AI in the right places, we help streamline operations, enhance customer experiences, and drive innovation throughout your business.",
  },
  {
    id: 2,
    img: mobiledevelopmentimage,
    image: smobiledevelopmentimage,
    title: "Mobile App Development",
    description:
      "Your customers are mobile, and so should your business. Our team specializes in creating high-performance mobile apps for iOS and Android that are designed with the user in mind. Whether it’s a simple app for your customers or a more complex enterprise solution, we ensure that your app performs seamlessly, looks great, and offers a flawless experience. We build apps that engage users, boost your brand, and support your business goals while adapting to the ever-changing mobile landscape.",
  },
  {
    id: 3,
    img: webdevelopmentimage,
    image: swebdevelopmentimage,
    title: "Website Development",
    description:
      "Your website is often the first point of contact for potential customers. That’s why we focus on building websites that don’t just look good but also provide an exceptional user experience. From responsive design to secure, scalable development, we create websites that serve as powerful tools to attract and convert visitors. Whether you need an e-commerce platform, a corporate site, or a custom web application, we tailor each project to meet your specific needs and ensure your online presence reflects your brand while driving measurable results.",
  },
  {
    id: 4,
    img: uiuximage,
    image: suiuximage,
    title: "UX & UI Design",
    description:
      "Great design goes beyond aesthetics—it’s about creating a seamless user experience. Our UX/UI team is dedicated to designing interfaces that are intuitive, easy to navigate, and aligned with your business goals. We combine research, testing, and feedback to ensure users enjoy a smooth and engaging journey with your product, leading to increased satisfaction and retention. Every design decision is purpose-driven, delivering both beauty and functionality.",
  },
  {
    id: 5,
    img: customsoftwareimage,
    image: scustomsoftwareimage,
    title: "Custom Software Development",
    description:
      "Every business is unique, which is why off-the-shelf software often doesn’t meet your needs. At [Your Company Name], we create custom software solutions that are built to fit your business processes, goals, and vision. From internal tools to customer-facing applications, we develop software that’s as unique as your business. We work with you every step of the way—from planning and development to implementation and support—to ensure your custom solution helps your business run efficiently and effectively.",
  },
  {
    id: 6,
    img: dataanalyticsimage,
    image: sdataanalyticsimage,
    title: "Data Analysis",
    description:
      "In today’s data-driven world, analyzing and leveraging data is a critical asset. We help you turn raw data into actionable insights that drive smarter decisions and better outcomes. Our services include data mining, visualization, predictive analytics, and real-time reporting, empowering you to understand trends, optimize performance, and improve your bottom line. By unlocking the full potential of your data, we provide the tools and insights needed to stay competitive.",
  },
  {
    id: 7,
    img: iotimage,
    image: siotimage,
    title: "Internet of Things (IoT)",
    description:
      "The Internet of Things (IoT) is connecting the world in unimaginable ways. We help businesses leverage IoT technology to create smarter products, streamline operations, and enhance customer experiences. From IoT sensors in manufacturing to connected devices in healthcare, we design solutions that give you real-time insights, improve automation, and drive operational efficiency. Our IoT solutions are tailored to meet the specific needs of your industry, helping you stay ahead with innovative technologies.",
  },
  {
    id: 8,
    img: roboticautomationimage,
    image: sroboticautomationimage,
    title: "Robotic Process Automation (RPA)",
    description:
      "Repetitive tasks are the enemy of productivity, and RPA can help eliminate them. RPA is a powerful tool for automating routine processes such as data entry, invoice processing, and customer service workflows, allowing your team to focus on more strategic work. At [Your Company Name], we design RPA solutions that integrate seamlessly with your existing systems, helping reduce costs, minimize errors, and boost efficiency across your organization.",
  },
  {
    id: 9,
    img: roboticautomationimage,
    image: sroboticautomationimage,
    title: "Augmented Reality (AR) & Virtual Reality (VR)",
    description:
      "AR and VR are revolutionizing the way businesses interact with customers and employees. From immersive product experiences to virtual training simulations, we help you harness the power of AR and VR to engage your audience in new and exciting ways. These technologies can be used for marketing, education, training, or entertainment. Whether creating virtual product demos or interactive customer experiences, we develop solutions that take full advantage of AR and VR's potential.",
  },
  {
    id: 10,
    img: gamedevelopmentimage,
    image: sgamedevelopmentimage,
    title: "Game Development",
    description:
      "Games can be powerful tools for brand engagement, learning, and marketing. At [Your Company Name], we specialize in developing interactive, fun, and engaging games that deliver value to both users and businesses. Whether it’s a mobile game, VR experience, or gamified web app, we work closely with you to design a game that aligns with your objectives and keeps users coming back. Our game development process includes everything from concept and design to deployment.",
  },
  {
    id: 11,
    img: gamedevelopmentimage,
    image: sgamedevelopmentimage,
    title: "Blockchain Development",
    description:
      "Blockchain is redefining trust, transparency, and security in business. At [Your Company Name], we offer blockchain development services that help businesses create secure, decentralized systems and applications. Whether building smart contracts, developing DApps, or integrating blockchain into your processes, we guide you through the development lifecycle. With blockchain, we help create secure, transparent systems that boost confidence and improve data integrity.",
  },
  {
    id: 12,
    img: blockchainimage,
    image: sblockchainimage,
    title: "Cloud Service Development",
    description:
      "The cloud is the foundation for modern business operations. At [Your Company Name], we provide cloud service development solutions that help you take full advantage of the cloud. Whether migrating to the cloud, optimizing cloud-based workflows, or developing custom cloud applications, we ensure that your systems are scalable, secure, and efficient. We work with leading cloud providers to help you build an agile cloud infrastructure that supports your growth.",
  },
];

export const routes = [
  { path: "/", name: "home" },
  { path: "/about-us", name: "about us" },
  { path: "/services", name: "services" },
  { path: "/contact-us", name: "contact us" },
];

export const meetCompanyDetails = [
  {
    id: 1,
    img: meetcompanyimageone,
    heading: "Innovative Technology Solutions",
    description:
      "At RETOOL, we blend creativity with technical expertise to deliver cutting-edge technology services. From AI-powered tools to user-friendly mobile applications, we craft scalable solutions tailored to your business needs, ensuring efficiency, innovation, and long-term success.",
  },
  {
    id: 2,
    img: meetcompanyimagetwo,
    heading: "Empowering Businesses with AI",
    description:
      "RETOOL transforms challenges into opportunities with AI-driven solutions and tailored strategies. Our scalable platforms and intuitive applications are designed to adapt as your business evolves, driving innovation and growth in the digital age.",
  },
  {
    id: 3,
    img: meetcompanyimagethree,
    heading: "Driving Success with Collaboration",
    description:
      "RETOOL specializes in delivering customized, future-ready technology solutions. Our team partners with clients to craft AI-powered tools and mobile applications that enhance operations, foster innovation, and unlock new business potential.",
  },
];

export const companyProcess = [
  {
    id: 1,
    img: Finetunedsolutionimg,
    title: "Fine-tuned solution",
    description:
      "At RETOOL, we know that every business is unique. That’s why we take the time to understand your specific needs and challenges. Whether you need a custom software application or an AI-driven analytics platform, we craft solutions that fit seamlessly into your operations",
  },
  {
    id: 2,
    img: Innovationfocused,
    title: "Innovation-focused",
    description:
      "The digital landscape is always changing, and so are we. Our focus on continuous learning and innovation keeps us ahead of the curve. We leverage the latest technologies and methods to provide you with cutting-edge solutions.",
  },
  {
    id: 3,
    img: Scalableframeworks,
    title: "Scalable frameworks",
    description:
      "As your business evolves, your technology should too. Our solutions are designed to be scalable, allowing you to grow and adapt without the limitations of outdated systems.",
  },
];

export const teamMembers = [
  { id: 1, name: "Brooklyn Simmons", position: "Developer" },
  { id: 2, name: "Guy Hawkins", position: "UI/UX Developer" },
  { id: 3, name: "jhon dani", position: "Developer" },
  { id: 4, name: "Brooklyn Simmons", position: "Developer" },
  { id: 1, name: "Brooklyn Simmons", position: "Developer" },
  { id: 2, name: "Guy Hawkins", position: "UI/UX Developer" },
  { id: 3, name: "jhon dani", position: "Developer" },
  { id: 4, name: "Brooklyn Simmons", position: "Developer" },
];
export const faqItems = [
  {
    id: 1,
    question: "How can I get in touch with RETOOL?",
    answer: `You can reach out to us through the contact form on this page, or by emailing us directly at info@example.com. Alternatively, you can also call us at +91-6300021466 for immediate assistance.`,
  },
  {
    id: 2,
    question: "How long will it take for someone to respond to my inquiry?",
    answer: `We aim to respond to all inquiries within 24–48 hours on business days. If you need urgent assistance, please indicate so in your message, and we’ll prioritize your request.`,
  },
  {
    id: 3,
    question: "What types of services does RETOOL offer?",
    answer: `RETOOL offers a wide range of services including:
      - Artificial Intelligence (AI)
      - Mobile App Development
      - Website Development
      - UI/UX Design
      - Custom Software Development
      - Data Analytics
      - Internet of Things (IoT)
      - Robotic Process Automation (RPA)
      - Augmented Reality (AR) & Virtual Reality (VR)
      - Game Development
      - Blockchain Solutions
      - Cloud Services
      - Cybersecurity
    For more detailed information, please visit our Services page.`,
  },
  {
    id: 4,
    question: "How do I schedule a consultation or meeting with RETOOL?",
    answer: `You can schedule a meeting by filling out the contact form on this page or by sending us an email at <strong>info@example.com</strong>. We’ll get back to you with available time slots.`,
  },
  {
    id: 5,
    question: "Do you offer support for ongoing projects?",
    answer: `Yes, we provide support for all ongoing projects. Whether you need help with updates, maintenance, or troubleshooting, our team is ready to assist you.`,
  },
  {
    id: 6,
    question: "Can I request a quote for a specific service?",
    answer: `Yes, we provide custom quotes based on your requirements. Please fill out the contact form with details about the service you need, and we’ll send you an estimate as soon as possible.`,
  },
  {
    id: 7,
    question: "Do you offer free consultations?",
    answer: `Yes, we offer a free initial consultation to better understand your needs and discuss potential solutions. Reach out to schedule your consultation!`,
  },
  {
    id: 8,
    question: "Where is RETOOL located?",
    answer: `RETOOL operates remotely, and our team works from various locations around the world. We’re fully capable of supporting clients globally.`,
  },
  {
    id: 9,
    question: "What industries do you specialize in?",
    answer: `RETOOL has worked with clients across multiple industries, including:
      - Tech
      - Healthcare
      - Finance
      - Retail
      - Entertainment
    We tailor our solutions to fit the unique needs of each industry.`,
  },
  {
    id: 10,
    question: "Can I work with RETOOL from any country?",
    answer: `Absolutely! RETOOL works with clients globally. Thanks to our remote setup, we are able to serve businesses in different time zones and regions efficiently.`,
  },
  {
    id: 11,
    question: "How do I track the progress of my project with RETOOL?",
    answer: `We provide regular project updates through email, meetings, or our project management platform. You will have access to track the status and milestones of your project.`,
  },
  {
    id: 12,
    question: "What makes RETOOL different from other companies?",
    answer: `At RETOOL, we focus on delivering cutting-edge, custom solutions tailored to your business needs. Our approach combines deep industry expertise with a focus on innovation and customer satisfaction, ensuring we always exceed expectations.`,
  },
];

export const testimonials = [
  {
    id: 1,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Emily R.",
    position: "CEO, TechSmart Innovations",
    says: "Partnering with [Your Company Name] was a game changer for us. They helped us implement a powerful AI-driven system for predictive analytics that completely transformed our decision-making process. The team took the time to truly understand our needs and delivered a solution that exceeded our expectations. Their support and expertise have been instrumental in driving our business forward.",
  },
  {
    id: 2,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "David S.",
    position: "Co-Founder, FitLife Mobile",
    says: "[Your Company Name] stands out. They designed and developed a mobile app for our business that not only looks great but also functions flawlessly. The end product has allowed us to better engage with our customers and streamline our internal processes. The team's attention to detail and ongoing support made the entire experience seamless.",
  },
  {
    id: 3,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Amanda T.",
    position: "Marketing Director, Bloom & Co.",
    says: "Working with [Your Company Name] was an absolute pleasure. They didn’t just build us a website—they built a digital experience that perfectly represents our brand. Their expertise in both design and functionality was evident, and the site’s performance has far exceeded our expectations. We’re thrilled with the outcome and are excited to continue working with them.",
  },
  {
    id: 4,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Sophia H.",
    position: "Product Manager, GreenSprout",
    says: "[Your Company Name] nailed it. Their team worked closely with us to design an interface that is both visually stunning and incredibly intuitive. We’ve received positive feedback from users ever since the launch, and we’ve seen a significant improvement in engagement. The collaboration was smooth, and they truly understood our vision.",
  },
  {
    id: 5,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Ryan L.",
    position: "Operations Manager, GlobalTech Enterprises",
    says: "Custom software was a must for our business, and [Your Company Name] delivered. They developed an ERP solution tailored to our exact needs, and it has revolutionized how we manage our operations. The system integrates seamlessly with our existing tools and has significantly improved our efficiency. The team was professional, responsive, and incredibly knowledgeable throughout the entire process.",
  },
  {
    id: 6,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Jasmine W.",
    position: "Chief Strategy Officer, MarketSense Analytics",
    says: "Thanks to [Your Company Name], we’ve been able to transform our approach to data. Their data analysis team helped us dig deeper into our customer trends and gave us valuable insights that have completely shifted our strategy. They provided not just data, but actionable recommendations that have proven essential for our decision-making.",
  },
  {
    id: 7,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Carlos D.",
    position: "CEO, ProManufacture Ltd.",
    says: "Implementing IoT with [Your Company Name] has completely changed how we manage our production line. Their team developed a custom IoT system that gives us real-time data and insights, helping us increase efficiency and reduce costs. The entire process, from initial consultation to final implementation, was smooth, and the results speak for themselves. We couldn’t be happier with the outcome.",
  },
  {
    id: 8,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Maria G.",
    position: "CFO, TechSolutions Group",
    says: "Automating our workflows with RPA has been one of the best decisions we’ve made. They helped us implement automation in several areas, such as data entry and customer onboarding, which has dramatically improved our speed and accuracy. Their team made sure the process was simple and the integration with our existing systems was seamless. The impact on our productivity has been remarkable.",
  },
  {
    id: 9,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Liam C.",
    position: "Head of Marketing, RealEstate Pro",
    says: "[Your Company Name] helped us bring our vision of AR and VR experiences to life. They created interactive demos and virtual training programs that our clients and team members love. It’s been a great way for us to stand out in the market and engage with users in a completely new way. The team’s creativity and technical expertise were crucial to the success of the project.",
  },
  {
    id: 10,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Tasha P.",
    position: "Co-Founder, PlayPlus Studios",
    says: "Creating a mobile game with [Your Company Name] was an exciting and rewarding experience. From the initial concept to the final launch, their team guided us every step of the way. The game has been a hit with users, and we’ve seen great engagement from players. Their team’s dedication, creativity, and technical know-how made all the difference in turning our idea into a successful product.",
  },
  {
    id: 11,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Samuel K.",
    position: "Founder, SecureTrade Ltd.",
    says: "Blockchain technology was new to us, but [Your Company Name] made the transition seamless. They helped us develop a blockchain-based system that improves security and transparency across our transactions. The team’s deep understanding of the technology, along with their clear communication, ensured that everything went smoothly. We now have a secure, decentralized system that adds tremendous value to our operations.",
  },
  {
    id: 12,
    img: testimonialsuserimage,
    starimg: fullstart,
    name: "Grace M.",
    position: "CTO, InnovateTech Solutions",
    says: "We turned to [Your Company Name] to help us migrate to the cloud, and it’s been one of the best decisions we’ve made. Their team took the time to understand our unique needs and provided a cloud solution that is secure, scalable, and highly efficient. Our team can now access data and apps from anywhere, improving collaboration and productivity across the board.",
  },
];

export const webDevelopmentServices = [
  {
    id: 1,
    title: "E-commerce Websites",
    img: ecommerce,
    description:
      "Fuel your online business with custom e-commerce solutions. Our platforms are designed to enhance user engagement, optimize operations, and boost revenue, creating a seamless shopping experience for your customers.",
  },
  {
    id: 2,
    title: "Social Media Platforms",
    img: socialmedia,
    description:
      "Build a dynamic social media platform that fosters community and engagement. We create feature-rich, user-friendly websites that help your brand thrive and foster customer loyalty.",
  },
  {
    id: 3,
    title: "Landing Pages",
    img: landingpage,
    description:
      "Crafting high-impact landing pages that capture attention and drive results. Our designs emphasize your brand’s message, encouraging action and maximizing conversions.",
  },
  {
    id: 4,
    title: "Custom Web Solutions",
    img: custom,
    description:
      "Providing custom website development tailored to your unique business objectives. Our solutions combine cutting-edge design with functionality, delivering a powerful and memorable online experience for your audience.",
  },
];

// app development services
export const appDevelopmentServices = [
  {
    id: 1,
    title: "iOS App Development",
    img: iosapp,
    description:
      "Our skilled developers work with both Swift and Objective-C to build robust and efficient apps. We optimize your app for performance, speed, and responsiveness.",
  },
  {
    id: 2,
    title: "Android App Development",
    img: android,
    description:
      "We design and develop tailor-made Android apps that align with your vision and business goals.Proficient in both Java and Kotlin, we build robust and efficient apps.",
  },
  {
    id: 3,
    title: "Flutter App Development",
    img: flutter,
    description:
      "Flutter allows you to build apps for both iOS and Android using a single codebase. While Flutter's default web app architecture presents challenges, we can overcome them with smart strategies. ",
  },
  {
    id: 4,
    title: "Hybrid App Development",
    img: hybrid,
    description:
      "We build feature-rich hybrid apps tailored to your business needs. We create mobile-friendly apps that work seamlessly across iOS and Android devices.",
  },
];

export const whyChooseUs = [
  {
    id: 1,
    icon: mechanical,
    heading: "Innovation at Our Core: Pioneering the Future",
    description:
      "We don't merely follow trends; we lead the charge in shaping the future. By harnessing cutting-edge technology and visionary strategies, we craft solutions that redefine the boundaries of possibility. Our commitment to innovation drives us to explore new frontiers, empowering your business with transformative tools and insights that set you apart in a competitive landscape. Join us as we turn bold ideas into reality, positioning your organization at the forefront of the digital revolution.",
  },
  {
    id: 2,
    icon: target,
    heading: "Customer-Centric Approach: Your Vision, Our Mission",
    description:
      "At RETOOL, you are not just a client; you are our top priority. We are dedicated to forging strong partnerships that allow us to deeply understand your unique needs and aspirations. Our team collaborates with you every step of the way, ensuring that we deliver tailored solutions that seamlessly integrate with your business. With a focus on personalization and excellence, we empower you to achieve your goals and thrive in today’s dynamic marketplace. Your success is our mission, and we are here to make it happen.",
  },
  {
    id: 3,
    icon: expert,
    heading: "Comprehensive Expertise: A Multidisciplinary Powerhouse",
    description:
      "Our diverse team of specialists represents a fusion of knowledge and experience across multiple domains, including artificial intelligence, cybersecurity, cloud computing, and more. This multidisciplinary expertise allows us to tackle even the most complex challenges with confidence and precision. By leveraging our collective skills, we develop innovative solutions tailored to your specific needs, ensuring you receive the highest level of service and support. Trust in our comprehensive expertise to propel your business forward and navigate the ever-evolving landscape of technology with assurance and agility.",
  },
  {
    id: 4,
    icon: cogwheel,
    heading: "Scalable and Future-Proof Solutions",
    description:
      "We don’t just build for today—we build for tomorrow. Our solutions are designed to grow with your business, ensuring long-term success and adaptability in an ever-changing market.",
  },
];

export const portfolioDataWeb = [
  {
    id: 1,
    name: "5g Homes",
    image: ghomes,
  },
  {
    id: 2,
    name: "Autopilot",
    image: Autopilot,
  },
  {
    id: 3,
    name: "Bayut",
    image: bayut,
  },
  {
    id: 4,
    name: "Cold Creekcap",
    image: coldcreekcap,
  },
  {
    id: 5,
    name: "College Nutritionnist",
    image: collegenutritionnist,
  },
  {
    id: 6,
    name: "Leadership",
    image: leadership,
  },
  {
    id: 7,
    name: "Menissa Caterings",
    image: menissacaterings,
  },
  {
    id: 7,
    name: "Think Reality",
    image: thinkreality,
  },
];

export const portfolioDataApp = [
  {
    id: 1,
    name: "Cryptopadie",
    image: cryptopadie,
  },
  {
    id: 2,
    name: "Doctorplus",
    image: doctorplus,
  },
  {
    id: 3,
    name: "Dubai Travel Guide",
    image: dubaitravelguide,
  },
  { id: 4, name: "Elora Hair Palour", image: elorahairpalour },
  {
    id: 5,
    name: "Evans Francis",
    image: evansfrancis,
  },
  {
    id: 6,
    name: "House Of Deliverance",
    image: houseofdeliverance,
  },
  {
    id: 7,
    name: "Mybitsshop",
    image: mybitsshop,
  },
  {
    id: 8,
    name: "Namaz",
    image: namaz,
  },
  {
    id: 9,
    name: "Potea",
    image: potea,
  },
  {
    id: 9,
    name: "Rentop",
    image: rentop,
  },
];
